...
export function CreateEventFlow({ flowMode = "auto", onPublish }: CreateEventFlowProps) {
  ...
  function goNext() {
    ...
    if (step === 0) {
      ...
      flash("Flyer looks great");
      setStep(1);
      return;
    }
    if (step === 1) {
      if (!validateStep1()) return;
      flash("Schedule's set");
      setStep(2);
    }
  }
              ...
              <div className="mx-auto flex max-w-md items-center gap-2 sm:max-w-lg">
                {step > 0 && !showAllInOne && (
                  <button
                    type="button"
                    onClick={goBack}
                    className="flex h-12 w-12 shrink-0 items-center justify-center rounded-full border border-white/15 text-zinc-400 transition hover:border-white/30 hover:text-white"
                    aria-label="Back"
                  >
                ...
                {step < 2 ? (
                  ...
                  <button
                    type="button"
                    onClick={goNext}
                    className="group flex h-12 min-w-0 flex-1 items-center justify-center gap-2 rounded-full bg-gradient-to-r from-white to-zinc-100 text-[12px] font-semibold uppercase tracking-[0.14em] text-black shadow-[0_0_32px_-8px_rgba(255,255,255,0.35)] transition hover:shadow-[0_0_40px_-6px_rgba(75,250,148,0.35)]"
                  >
                    Next
